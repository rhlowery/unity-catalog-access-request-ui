...
export const GitAdapter = {
    ...
    async upsertRequest(request: any, config: any) {
        ...
        // If status is APPROVED/DENIED/EXPIRED, we handle Merge/Close lifecycle
        if (['APPROVED', 'DENIED', 'EXPIRED', 'REVOKED'].includes(request.status)) {
            if (state.branches[branchName]) {
                ...
                // Already merged/closed, just update record in merged list
                const idx = state.merged.findIndex((r: any) => r.id === request.id);
                if (idx !== -1) {
    ...
    async getGrants(object: any, config: any) {
        ...
        const grants = [];
        allReqs
            .filter((r: any) => ['APPROVED', 'MERGED'].includes(r.status)) // In GitOps, MERGED is equivalent to APPROVED
            .forEach((r: any) => {
                const targetsObject = r.requestedObjects.some((o: any) => o.id === object.id);
                if (targetsObject) {
                    r.principals.forEach((principal: any) => {
                        grants.push({
                            principal: principal,
