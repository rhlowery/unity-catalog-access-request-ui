...
defineParameterType({
  ...
  transformer: (value: string) => {
    try {
      ...
      return {
        frameOptions: parsed?.frameOptions || 'DENY',
        imgSrc: parsed?.imgSrc || 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAy...',
        sandbox: parsed?.sandbox || false,
        reportOnly: parsed?.reportOnly || false
      };
    } catch {
      return {
        frameOptions: 'DENY',
        imgSrc: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAy...',
        sandbox: false,
        reportOnly: false
      };
    }
...
Given('the login attempt count is {loginAttempts}', (attempts: number) => {
  cy.window().then((win) => {
...
Then('the request should be updated', () => {
  cy.window().then((win) => {
    if (win.approvers && win.approvers.length > 0) {
